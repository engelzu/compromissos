...
export function renderTable(compromissos, filterArea, search, currentPage = 1, filterStatus = '', filterResponsavel = '', filterReuniao = '') {
  ...
  if (filtered.length === 0) {
    return `<div class="bg-white rounded-lg shadow-sm p-12 text-center">...</div>`; // Renderiza a mensagem de vazio
  }
        ...
        <table class="w-full">
          <thead class="table-header">
            <tr>
              ...
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Observação</th> <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Ações</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            ${pagedItems.map(c => `
              <tr class="table-row">
                ...
                <td class="px-4 py-3"><div class="text-xs text-gray-500 line-clamp-2 w-40">${c.observacao || '-'}</div></td> <td class="px-4 py-3 whitespace-nowrap">
